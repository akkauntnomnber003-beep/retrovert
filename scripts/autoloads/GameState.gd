...
var auto_aim: bool = false               # toggleable AI auto-target nearest enemy
...
func reset_run() -> void:
	...
	auto_aim = bool(settings.get("auto_aim", false))
...
func compute_tear_lifetime() -> float:
	# Tear travels for ~player_tear_range pixels worth of time. We also let
	# the abstract Isaac "range index" (player_range) modulate the result —
	# every +1 in range index extends life by ~12 %.
	var spd: float = max(40.0, player_tear_speed * player_shot_speed)
	var dist: float = max(60.0, player_tear_range) * (1.0 + (player_range - 6.5) * 0.12)
	var luck_bonus: float = 1.0 + clamp(player_luck, -1.0, 2.0) * 0.03
	return clampf(dist / spd * luck_bonus, 0.25, 4.0)


func compute_tear_scale() -> float:
	return clampf(0.6 + player_damage * 0.12, 0.55, 2.4)
