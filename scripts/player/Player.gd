...
# Also draws a thin aim indicator under the player to make it obvious
# which direction the next tear will fly.
extends CharacterBody2D

const FRICTION := 1100.0
const ACCEL_MULT := 16.0
const AIM_LINE_LEN := 90.0
...
var auto_aim_dir: Vector2 = Vector2.ZERO  # populated by auto-aim each frame
...
func _update_auto_aim() -> void:
	auto_aim_dir = Vector2.ZERO
	if not GameState.auto_aim:
		return
	# Find nearest enemy within view_max.
	var tree := get_tree()
	if tree == null:
		return
	var best: Node = null
	var best_d2: float = GameState.view_max * GameState.view_max
	for e in tree.get_nodes_in_group("enemy"):
		...
		if d2 < best_d2:
			...
			best = e
	if best != null:
		auto_aim_dir = (best.global_position - global_position).normalized()
		if shoot_input.length() < 0.1:
			# Auto-fire when no manual shoot input is given.
			set_shoot_input(auto_aim_dir)
...
func _update_aim_indicator() -> void:
	...
	if not aim_indicator.is_connected("draw", _draw_aim):
		aim_indicator.draw.connect(_draw_aim)


func _draw_aim() -> void:
	if not is_alive:
		return
	var dir: Vector2 = shoot_input if shoot_input.length() > 0.1 else \
		(auto_aim_dir if auto_aim_dir.length() > 0.1 else facing_dir)
	if dir.length() < 0.1:
		dir = facing_dir
	dir = dir.normalized()
	var color := Color(0.95, 0.95, 1.0, 0.55) if is_shooting else \
		Color(0.65, 0.65, 0.75, 0.30)
	var origin := Vector2(0, -6)
	var end := origin + dir * AIM_LINE_LEN
	aim_indicator.draw_line(origin, end, color, 2.0, true)
	# Crosshair at the end of the line.
	aim_indicator.draw_circle(end, 4.0, color)
	aim_indicator.draw_circle(end, 2.0, Color(1, 1, 1, color.a))
...
func _apply_movement(delta: float) -> void:
	if move_input.length() > 0.05:
		# Clamp magnitude to 1.0 in case the touch joystick over-shoots.
		var m: float = clampf(move_input.length(), 0.0, 1.0)
		var target := move_input.normalized() * GameState.player_speed * m
