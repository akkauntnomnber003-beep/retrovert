...
func _build_globals() -> Dictionary:
	...
	# ── Chat / log overlay ────────────────────────────────────────────
	var mod_label: String = mod_info.name if mod_info else "mod"
	g["chat"] = func(args: Array):
		if args.is_empty(): return null
		ChatOverlay.mod(String(args[0]), mod_label)
		return null
	g["chat_log"] = g["chat"]
	g["chat_event"] = func(args: Array):
		if args.is_empty(): return null
		ChatOverlay.event(String(args[0]), mod_label)
		return null
	g["chat_error"] = func(args: Array):
		if args.is_empty(): return null
		ChatOverlay.error(String(args[0]), mod_label)
		return null
	g["chat_as"] = func(args: Array):
		if args.size() < 2: return null
		ChatOverlay.mod(String(args[1]), String(args[0]))
		return null

	# ── File system (sandboxed under user://mod_data/<mod_id>/) ───────
	g["file_read"] = func(args: Array) -> String:
		var p: String = _mod_data_path(String(args[0]))
	...
	g["file_write"] = func(args: Array) -> bool:
		...
		f.store_string(String(args[1]))
		return true
	g["file_exists"] = func(args: Array) -> bool:
		return FileAccess.file_exists(_mod_data_path(String(args[0])))
	g["file_list"] = func(args: Array) -> Array:
		var sub: String = String(args[0]) if not args.is_empty() else ""
		var p: String = _mod_data_path(sub)
		var out: Array = []
	...
	# ── HTTP / AI (callback-based — see NetAI.gd). ─────────────────────
	# Usage from RatScript:
	#   http_get("https://...", func(resp) chat(resp.body) end)
	#   http_post("https://...", "raw body", func(resp) ... end)
	#   ai("Hello", "default", func(text) chat(text) end)
	g["http_get"] = func(args: Array):
		var url: String = String(args[0])
		var cb = args[1] if args.size() > 1 else null
		NetAI.http_get(url, PackedStringArray(), _wrap_cb(cb))
		return null
	g["http_post"] = func(args: Array):
		var url2: String = String(args[0])
		var body2: String = String(args[1]) if args.size() > 1 else ""
		var cb2 = args[2] if args.size() > 2 else null
		NetAI.http_post(url2, body2, PackedStringArray(), _wrap_cb(cb2))
		return null
	g["ai"] = func(args: Array):
		var prompt: String = String(args[0]) if not args.is_empty() else ""
		var profile: String = String(args[1]) if args.size() > 1 \
			else "default"
		var cb3 = args[2] if args.size() > 2 else null
		NetAI.ai_chat(prompt, profile, _wrap_cb(cb3))
		return null
	g["ai_chat"] = g["ai"]
...
func _mod_data_path(sub: String) -> String:
	var id: String = mod_info.id if mod_info else "unknown"
	var safe := id.replace("..", "").replace("/", "_")
	var clean := sub.replace("..", "").lstrip("/")
	return "user://mod_data/%s/%s" % [safe, clean]


func call_lifecycle(fn_name: String, args: Array = []) -> Variant:
	# Lets ModLoader invoke `on_load()`, `on_tick(dt)`, `on_unload()` if
	# the script defined them as top-level functions.
	if not _globals.has(fn_name):
		return null
	return _invoke(_globals[fn_name], args)


func _wrap_cb(user_fn) -> Callable:
	# Turn a RatScript closure into a real Callable for NetAI to call.
	if user_fn == null:
		return Callable()
	return Callable(self, "_invoke_cb_one").bind(user_fn)


func _invoke_cb_one(arg, user_fn) -> void:
	# Bridge for one-arg callbacks (e.g. http response Dictionary).
	if user_fn == null:
		return
	_invoke(user_fn, [arg])
