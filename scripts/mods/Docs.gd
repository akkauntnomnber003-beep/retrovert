...
static func render_category(name: String) -> String:
	match name:
		...
		"Чат": return _chat_api()
		"Файлы (песочница)": return _files_api()
		...
		"Шаблоны модов": return _templates_overview()
		"Чит-фрагменты": return _cheat_fragments()
...
static func _chat_api() -> String:
	return """[b]Чат[/b]

Чат — это оверлей сверху экрана, через который игра, моды и события
сообщают игроку о происходящем. Сообщения буферизуются (последние 200
хранятся в [code]ChatLog.messages[/code]) и эмитятся сигналом
[code]EventBus.chat_message[/code].

[b]Формат:[/b] [code]Отправитель: текст[/code]
[b]Виды сообщений (kind):[/b] info, log, system, mod, error, enemy, item.

[b]Глобальные функции RatScript:[/b]

[code]chat_send(sender, text, kind, sender_id)[/code] — отправить любое сообщение
[code]chat_log(text)[/code] — сообщение от имени мода
[code]chat_error(text)[/code] — красное сообщение об ошибке
[code]chat_enemy(name, text, id)[/code] — от лица врага
[code]chat_item(name, text, id)[/code] — от лица предмета
[code]chat_clear()[/code] — очистить чат
[code]chat_show() / chat_hide()[/code] — управление видимостью

[b]Пример:[/b]
[code]chat_log("DarkRift инициализирован")
on("enemy_died") func(pos, type)
    chat_enemy(type, "Я пал...", type)
end[/code]

У отправителя может быть [b]sender_id[/b] — короткий ключ (uuid мода,
id предмета, id врага), по которому можно потом отфильтровать сообщения
через [code]ChatLog.find_by_sender_id(id)[/code]."""


static func _files_api() -> String:
	return """[b]Файлы — песочница мода[/b]

Каждый мод может писать и читать в собственную папку
[code]user://mods/<uuid>/data/[/code]. Доступ к файлам вне неё [b]заблокирован[/b]
(защита от вредоносных модов).

[b]Глобальные функции RatScript:[/b]

[code]file_read(path, default)[/code] — прочитать файл (или default)
[code]file_write(path, content)[/code] — записать
[code]file_list(subdir)[/code] — список файлов
[code]file_exists(path)[/code] — есть ли файл
[code]file_remove(path)[/code] — удалить

[b]Пример: сохранение состояния между запусками:[/b]
[code]let saved = file_read("kills.txt", "0")
let total = floor(saved) + 1
file_write("kills.txt", total)
chat_log("Всего убийств: " .. total)[/code]"""


static func _templates_overview() -> String:
	return """[b]Шаблоны модов[/b]

В Менеджере модов есть кнопка [b]Шаблоны[/b] — она устанавливает один
из готовых шаблонов в [code]user://mods/[/code]. После установки шаблона
вы можете открыть папку мода («Открыть папку» в деталях) и править
файлы локально.

[b]Доступные шаблоны:[/b]
  • [b]Предмет[/b] — регистрирует пассивный предмет, меняющий статы
  • [b]Враг + чат[/b] — спавнит особого врага и пишет в чат от его лица
  • [b]События[/b] — слушает enemy_died, room_cleared, player_hurt и логирует их
  • [b]Чат-ИИ[/b] — простая болталка: случайные реплики по таймеру
  • [b]Чит-фрагменты[/b] — набор готовых вызовов (+100 монет, бомбы и т.д.)

Все шаблоны хранятся в проекте по пути [code]res://mods/templates/*.retrovert[/code]
и доступны через [code]ModManager.tscn[/code]."""


static func _cheat_fragments() -> String:
	return """[b]Чит-фрагменты[/b]

Готовые однострочники, которые можно вставлять в любой [code].rrs[/code]
файл — например, в [code]on("floor_entered") func(n) ... end[/code].

[code]-- Денег
state.add_coins(99)

-- Хп до максимума
state.heal(state.get("player_hp_max"))
...
-- Бесконечный урон
state.set("player_damage", 99.0)

-- Очень быстрая стрельба
state.set("player_tear_rate", 12.0)

-- Хитрый авто-аим
state.set("auto_aim", true)

-- Шторм слёз каждые 2 сек
let t = now()
on("enemy_died") func(p, k) chat_log("kill: " .. k) end[/code]"""
